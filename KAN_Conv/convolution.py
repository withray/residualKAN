...
def kan_conv2d(matrix: torch.Tensor, kernel: torch.nn.Module, kernel_side: int,
    ...
    unfolded_matrix = unfold(matrix)  # Shape: (batch_size, n_channels * kernel_side * kernel_side, h_out * w_out)
    unfolded_matrix = unfolded_matrix.view(batch_size, n_channels, kernel_side * kernel_side, h_out * w_out)
    conv_groups = unfolded_matrix.permute(0, 1, 3, 2)  # Correcting the permutation
    
    # Apply the kernel in a vectorized manner
...
def multiple_convs_kan_conv2d(matrix: torch.Tensor, kernels: List[torch.nn.Module], kernel_side: int,
    ...
    unfolded_matrix = unfold(matrix)  # Shape: (batch_size, n_channels * kernel_side * kernel_side, h_out * w_out)
    unfolded_matrix = unfolded_matrix.view(batch_size, n_channels, kernel_side * kernel_side, h_out * w_out)
    conv_groups = unfolded_matrix.permute(0, 1, 3, 2)  # Shape: (batch_size, n_channels, h_out * w_out, kernel_side * kernel_side)
    
    # Apply each kernel in a vectorized manner
    results = []
    ...
    # Concatenate results from all kernels
    matrix_out = torch.cat(results, dim = 1)  # Concatenate along the channel dimension

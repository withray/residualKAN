...
class ResKNet(nn.Module):
    def __init__(self, num_classes = 100, version = "resnet18", kan_type = "chebyshev", pretrained = False, main_conv = "resnet", fcl = "resnet",
        ...
        self.dual_se_blocks = nn.ModuleList([
            nn.ModuleList([self._make_se_block(channels[0], reduction = 16), self._make_se_block(channels[0], reduction = 16)]),
            nn.ModuleList([self._make_se_block(channels[1], reduction = 16), self._make_se_block(channels[1], reduction = 16)]),
            nn.ModuleList([self._make_se_block(channels[2], reduction = 16), self._make_se_block(channels[2], reduction = 16)]),
            nn.ModuleList([self._make_se_block(channels[3], reduction = 16), self._make_se_block(channels[3], reduction = 16)])
        ])
    ...
    def apply_mechanism(self, out, residual, layer_index, mechanism):
        ...
        elif mechanism == "dual_se":
            main_weight = self.dual_se_blocks[layer_index][0](residual)
            residual_weight = self.dual_se_blocks[layer_index][1](out)
            self._add_params(self.dual_se_blocks[layer_index])
            return out * main_weight + residual * residual_weight

...
class RKAN_WideResNet(nn.Module):
    def __init__(self, num_classes = 1000, version = "wide_resnet50_2", kan_type = "chebyshev", pretrained = False, reduce_factor = [2, 2, 2, 2], n_convs = 1,
        ...
        # KAN normalization
        if self.inv_bottleneck:
            self.kan_bn = nn.ModuleList([nn.BatchNorm2d(ch * self.inv_factor) for i, ch in enumerate([64] + channels[:-1])])
        else:

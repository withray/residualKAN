...
class RKAN_RegNet(nn.Module):
    def __init__(self, num_classes = 1000, version = "regnet_y_3_2gf", kan_type = "chebyshev", pretrained = False, reduce_factor = [2, 2, 2, 2], n_convs = 1,
                 mechanisms = [None, None, None, "addition"], spline_order = (3, 2), grid_size = (3, 2), inv_bottleneck = False, inv_factor = 4):
        ...
        self.kan_conv1 = nn.ModuleList([
            KAN_Convolutional_Layer(n_convs = n_convs, kernel_size = (3, 3), stride = (2, 2), padding = (1, 1),
        ...
        if inv_bottleneck:
            self.conv_reduce = nn.ModuleList([
                nn.Conv2d(32, 32 * self.inv_factor, kernel_size = 1, stride = 1, bias = False),
            ...
            self.conv_expand = nn.ModuleList([
                nn.Conv2d((32 * self.inv_factor) * n_convs, channels[0], kernel_size = 1, stride = 1, bias = False),
            ...
            self.conv_expand = nn.ModuleList([
                nn.Conv2d((32 // reduce_factor[0]) * n_convs, channels[0], kernel_size = 1, stride = 1, bias = False),
        ...
        if self.inv_bottleneck:
            self.kan_bn = nn.ModuleList([nn.BatchNorm2d(ch * self.inv_factor) for i, ch in enumerate([32] + channels[:-1])])
        else:
